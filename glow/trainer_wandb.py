        ...
        for epoch in range(self.n_epoches):
            ...
            for i_batch, batch in enumerate(progress):
                ...
                if self.global_step % self.scalar_log_gaps == 0:
                    ...
                    wandb.log({"lr": lr, "epoch": epoch+i_batch/len(self.data_loader)})
                # get batch data
                for k in batch:
                ...
                if self.global_step % self.scalar_log_gaps == 0:
                    ...
                    wandb.log({"loss_generative": loss_generative, "epoch": epoch+i_batch/len(self.data_loader)})
                    if self.y_condition:
                        # self.writer.add_scalar("loss/loss_classes", loss_classes, self.global_step)
                        wandb.log({"loss_classes": loss_classes, "epoch": epoch+i_batch/len(self.data_loader)})
                ...
                if self.max_grad_norm is not None and self.max_grad_norm > 0:
                    ...
                    if self.global_step % self.scalar_log_gaps == 0:
                        ...
                        wandb.log({"grad_norm": grad_norm, "epoch": epoch+i_batch/len(self.data_loader)})
                # step
                self.optim.step()
                ...
                if self.global_step % self.plot_gaps == 0:
                    ...
                    for bi in range(min([len(img), 4])):
                        ...
                        wandb.Image({"0_reverse/{}".format(bi): torch.cat((img[bi], batch["x"][bi]), dim=1), "epoch": epoch+i_batch/len(self.data_loader)})
                        if self.y_condition:
                            # self.writer.add_image("1_prob/{}".format(bi), plot_prob([y_pred[bi], y_true[bi]], ["pred", "true"]), self.global_step)
                            wandb.Image({"1_prob/{}".format(bi): plot_prob([y_pred[bi], y_true[bi]], ["pred", "true"]), "epoch": epoch+i_batch/len(self.data_loader)})
                ...
                if hasattr(self, "inference_gap"):
                    if self.global_step % self.inference_gap == 0:
                        ...
                        wandb.Image({"2_sample/{}".format(bi): img[bi], "epoch": epoch+i_batch/len(self.data_loader)})
